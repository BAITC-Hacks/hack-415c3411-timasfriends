...
Ответ содержит `conversationId`, `message`, `phase`, `aiMode`, `questions`, `draft`, `sources`, `missingFields`, `inputAccepted`, `validation`. Unity показывает сообщение и режим AI, переносит `draft` в редактор и предлагает открыть карточку при `phase:"draft_ready"`. `sources` сохраняются на сервере; отдельного просмотра цитат в UI нет.

`inputAccepted:false` означает просьбу исправить ввод: `message` объясняет проблему, `validation.issues` содержит `{field,code,message}`. Такой ответ не меняет принятый черновик и не продвигает три уточнения. Отклонённые сообщения сохраняются с отметкой в истории и исключаются из последующего извлечения. При неверном начальном описании повторяется вопрос о самой бизнес-проблеме. Неизвестный ответ на уточнение можно обозначить словами «не знаю».

После исходного описания помощник задаёт три вопроса **по одному на сообщение**. Ответ на первый сохраняется и вызывает второй вопрос, ответ на второй — третий. В каждом ответе с `phase:"clarifying"` массив `questions` содержит один вопрос, а `draft` — накопленные сведения. После ответа на третий вопрос сервер возвращает `phase:"draft_ready"`, `questions:[]` и черновик. «Не знаю» тоже завершает текущий шаг; неизвестное поле остаётся пустым. Четвёртого автоматического вопроса нет.

В SQLite сохраняются все сообщения, метаданные каждого вопроса, частичный черновик и цитаты. Обычный ответ в fallback относится к непосредственно предыдущему вопросу; явные строки `пользователи: …`, `данные: …`, `критерии успеха: …` также поддерживаются. После окончания трёх шагов свободные дополнения без меток остаются в истории, а поля можно редактировать в карточке. Перезапуск сервера сохраняет текущий шаг разговора.
...
Редактор вызывает `POST /api/tasks/preview` с `{draft,confirmedFields}` после паузы в наборе. В предварительном расчёте имена непустых полей передаются как предполагаемые подтверждения: панель **«Подтверждённые баллы»** показывает подтверждённый балл и возможную прибавку после успешной проверки. Запрос проверяет содержание, не публикует задачу и не сохраняет согласие. Неверные поля и заглушки не дают баллы. Ответ: `readiness`, `readinessLevel`, десять строк `scoreBreakdown`, `missingFields`, `validation`.

`validation` содержит `status` (`passed`, `rejected`, `unavailable`), `aiMode`, `issues:[{field,code,message}]`, `message`. Без настроенного AI работает локальная проверка явного мусора (`passed`/`rejected`, `aiMode:"fallback"`). Сбой настроенного AI даёт `unavailable` и нулевой предпросмотр до повторной проверки. В редакторе устаревший балл скрывается при правке, а ошибки и подсказки привязаны к полям; публикация доступна только после `passed` для текущего текста. Проверка одинакового содержания кэшируется на 60 секунд; неудачный вызов можно повторить сразу.

`POST /api/tasks` и `PATCH /api/tasks/{id}` проверяют весь присланный черновик независимо от клиента. Отклонение даёт 422 `content_invalid`, недоступность настроенного AI — 503 `content_review_unavailable`; тело ошибки сохраняет формат `{error:{code,message}}`. Результат проверки хранится в `TaskResponse.validation`. Проверка смысла не подтверждает фактическую достоверность данных.

Ошибки в `title` или `category` обнуляют рейтинг всей карточки до исправления, хотя эти поля не входят в сумму весов. Ошибки в остальных полях обнуляют вклад соответствующих полей. Поэтому отклонённая карточка не получает 100/100.

Публикация требует название, тему, адрес сервера и нажатие **«Подтвердить сведения»** после успешной проверки. Оно подтверждает все непустые поля формы; любая правка сбрасывает подтверждение. API поддерживает подтверждения отдельных полей через `confirmedFields`, но отдельных флажков для каждого поля в UI нет. Низкий балл не запрещает публикацию и отклик.
