...
Редактор вызывает `POST /api/tasks/preview` с `{draft,confirmedFields}` после паузы в наборе. В предварительном расчёте имена непустых полей передаются как предполагаемые подтверждения: панель **«После подтверждения»** показывает потенциальный балл. Запрос проверяет содержание, не публикует задачу и не сохраняет согласие. Неверные поля и заглушки не дают баллы. Ответ: `readiness`, `readinessLevel`, десять строк `scoreBreakdown`, `missingFields`, `validation`.
