...
2. Нажмите **«Карточка →»**. Дополните название, тему и известные сведения. Справа видны подтверждённый балл, возможная прибавка и вклад каждого поля. Неизвестные поля остаются пустыми.
...
Учитываются заполненные и подтверждённые поля. Заглушки вроде «не знаю» и «данных нет» не дают баллы. Оценка показывает полноту, а не достоверность или ценность задачи. Предпросмотр показывает потенциальную прибавку **после подтверждения**. Анимация подтверждения — визуальная обратная связь; опыт команды начисляет только сервер за выполненный этап.
...
Интеграция компилируется в Unity без ошибок Console. В Game view проверены логотип, выбор роли, подсветка «Мои отклики», выделение и копирование фрагмента сообщения, анимации +4/−4 при заполнении и удалении контакта, вход в демокаталог с активным фильтром. Эти действия не публиковали изменений задачи на сервере.
